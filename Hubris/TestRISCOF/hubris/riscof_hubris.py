...
class hubris(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config['PATH'] if 'PATH' in config else "","hubris")
    ...
    def initialise(self, suite, work_dir, archtest_env):
        ...
        self.sig_gen_cmd = 'python ' + self.sig_gen_exe + ' {elf} {bin} {output}'
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        # for hubris start building the '--isa' argument. the self.isa is dutnmae specific and may not be
        # useful for all DUTs
    ...
    def runTests(self, testList):
        ...
        for testname in testList:
            ...
            # name of the memory dump file for simulation
            memdump_file = 'my.memdump.bin'
            ...
            # function
            compilecmd = self.compile_cmd.format(testentry['isa'].lower(), self.xlen, test, elf, compile_macros)
            # substitute all variables in the extract command that we created in the initialize
            # function
            extractcmd = self.extract_text_cmd.format(self.xlen, elf, text)
            # substitute all variables in the sig gen command that we created in the initialize
            # function
            siggencmd = self.sig_gen_cmd.format(elf=elf, bin=memdump_file, output=sig_file)

            # if the user wants to disable running the tests and only compile the tests, then
            # the "else" clause is executed below assigning the sim command to simple no action
            # echo statement.
            if self.target_run:
                simcmd = self.dut_exe + ' +bin={0} +regstat={1} +memdump={2}'.format(text, logfile,
                                                                                     memdump_file)
            else:
                simcmd = 'echo "NO RUN"'
